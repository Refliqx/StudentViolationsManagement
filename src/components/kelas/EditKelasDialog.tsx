...
export default function EditKelasDialog({ 
    open, setOpen, form, handleChange, handleUpdate }: 
    { open: boolean; setOpen: (open: boolean) => void; form: {kelas: string}; handleChange: (key: string, value: string) => void; handleUpdate: () => void; }) {
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogContent className="w-full max-w-[min(90vw,300px)] max-h-[calc(100vh-10rem)] overflow-y-auto">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Silakan masukkan data kelas yang ingin diedit.
                    </DialogDescription>
                </DialogHeader>
